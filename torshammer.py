...
class httpPost(Thread):
    ...
    def _send_http_post(self, pause=10):
        ...
        self.socks.send(b"POST / HTTP/1.1\r\n"
                        b"Host: %s\r\n"
                        b"User-Agent: %s\r\n"
                        ...
                        (self.host.encode(), random.choice(useragents).encode()))
    ...
    def run(self):
        while self.running:
            while self.running:
                try:
                    if self.tor:     
                        self.socks.set_proxy(socks.PROXY_TYPE_SOCKS5, "127.0.0.1", 9050)
...
def usage():
    ...
    print(" -T|--tor Enable anonymizing through tor on 127.0.0.1:9050")
